...
describe("Database Connection and Schema", () => {
  ...
  describe("database initialization", () => {
    test("should create test_table on initialization", () => {
      ...
      expect((tableInfo as any).name).toBe("test_table");
    });

    test("should have correct table structure", () => {
      const tableInfo = testDb.query("PRAGMA table_info(test_table)").all() as any[];
    ...
    test("should insert initial record", () => {
      ...
      expect((records[0] as any).message).toBe('Database initialized successfully');
    });
  ...
  describe("database operations", () => {
    ...
    test("should query records", () => {
      ...
      const messages = (records as any[]).map(r => r.message);
    ...
    test("should handle parameterized queries", () => {
      ...
      const record = testDb.query('SELECT * FROM test_table WHERE message = ?').get(message) as any;
    ...
    test("should handle date operations", () => {
      ...
      const record = testDb.query('SELECT * FROM test_table WHERE message = ?').get('Date test') as any;
  ...
  describe("query performance and limits", () => {
    ...
    test("should handle ordering and filtering", () => {
      ...
      // Test ordering
      const orderedRecords = testDb.query('SELECT message FROM test_table WHERE message != ? ORDER BY message ASC').all('Database initialized successfully') as any[];
      const orderedMessages = orderedRecords.map(r => r.message);
      expect(orderedMessages).toEqual(['Apple', 'Banana', 'Cherry', 'Date']);

      // Test filtering
      const filteredRecords = testDb.query("SELECT * FROM test_table WHERE message LIKE ?").all('%a%') as any[];
